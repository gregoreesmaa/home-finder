...
def test_build_stdout_is_pure_table_json(tmp_path, monkeypatch, capsys):
    ...
    built/datex-restrictions/table.json and the web route parses
    rows+counts, so --build stdout must be exactly one JSON doc (the
    full table) and the human ok: status must ride stderr."""
    monkeypatch.setenv("DATEX_API_KEY", "test-key-not-real")
    calls: list = []
    _stub_urlopen_factory(monkeypatch, calls)
