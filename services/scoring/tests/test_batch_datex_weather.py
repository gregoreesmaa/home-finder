"""Tests for scripts/build/batch_datex_weather.py (issue #683).
...
from batch_datex_weather import build_table, main, parse_weather  # noqa: E402

FIXTURE_XML = """<?xml version="1.0" encoding="UTF-8"?>
<d2LogicalModel xmlns="http://datex2.eu/schema/2/2_0">
  <payloadPublication>
    <measurementSiteTable id="WEATHER_STATION_SITES">
      <measurementSite id="11111111-2222-4333-8444-555555555555">
        <measurementSiteLocation>
          <pointByCoordinates>
            <pointCoordinates>
              <latitude>59.4370</latitude>
              <longitude>24.7535</longitude>
            </pointCoordinates>
          </pointByCoordinates>
        </measurementSiteLocation>
      </measurementSite>
    ...
    <siteMeasurements>
      <measurementSiteReference id="11111111-2222-4333-8444-555555555555"/>
      <physicalQuantity>
        <parameterName>airTemperature</parameterName>
        <measuredValueBasicDataValue>-4.5</measuredValueBasicDataValue>
      </physicalQuantity>
      <physicalQuantity>
        <parameterName>roadSurfaceStatus</parameterName>
        <measuredValueBasicDataValue>ice</measuredValueBasicDataValue>
      </physicalQuantity>
    </siteMeasurements>
  </payloadPublication>
</d2LogicalModel>
...
def test_parses_fixture_station(tmp_path):
    rows = parse_weather(_write_fixture(tmp_path))
    assert len(rows) == 1
    assert rows[0]["lat"] == pytest.approx(59.4370)
    assert rows[0]["lon"] == pytest.approx(24.7535)
    assert rows[0]["measured"]["airTemperature"] == "-4.5"
    table = build_table(rows)
    assert table["counts"] == {"stations": 1, "measured": 1,
                               "located": 1}
...
def test_fixture_pull_then_build(tmp_path, monkeypatch, capsys):
    ...
    calls: list = []
    _stub_urlopen_factory(monkeypatch, calls)
    rc = main(["--pull", "--feeds", "weather", "--cache-dir", str(tmp_path)])
    assert rc == 0
    assert len(calls) == 1
    cached = list(tmp_path.glob("datex_weather_*.xml"))
    assert cached
    for p in cached:
        assert "test-key-not-real" not in p.read_text(encoding="utf-8")
    quota = json.loads(
        (tmp_path / "datex_weather_quota.json").read_text())
    ...
    rc = main(["--build", "--cache-dir", str(tmp_path)])
    assert rc == 0
    assert '"stations": 1' in capsys.readouterr().out
...
def test_fresh_cache_skips_request(tmp_path, monkeypatch):
    ...
    calls: list = []
    _stub_urlopen_factory(monkeypatch, calls)
    args = ["--pull", "--feeds", "weather", "--cache-dir", str(tmp_path)]
    assert main(args) == 0
    n_first = len(calls)
    assert main(args) == 0
...
def test_quota_cap_refuses(tmp_path, monkeypatch, capsys):
    ...
    calls: list = []
    _stub_urlopen_factory(monkeypatch, calls)
    (tmp_path / "datex_weather_quota.json").write_text(
...
def test_transport_error_never_cached(tmp_path, monkeypatch):
    ...
    rc = main(["--pull", "--feeds", "weather", "--cache-dir", str(tmp_path)])
    assert rc == 1
    assert list(tmp_path.glob("datex_weather_*.xml")) == []
