"""oberhaus.ee listings adapter (Ober-Haus agency portal).
...
https://www.oberhaus.ee/robots.txt with the daily UA timed out (curl rc=28,
http=000). Search markup was NOT fetched; SELECTORS below are hand-built
from observed public markup knowledge and tests/fixtures/oberhaus_search.html
is a synthetic offline sample (not a scraped dump). Re-verify SEARCH_URL +
selectors with one polite probe before enabling cron.
"""
...
def fetch_search_html(query: str = "", page_limit: int = 1, timeout: float = 20.0) -> str:
    """Single page of oberhaus.ee search HTML. Keep page_limit small; cron, don't hammer."""
    params = {"page": 1}
    if query:
        params["q"] = query
...
def _parse_card(card_id: str, body: str) -> dict:
    ...
    return normalize_listing(
        {
            "id": "oberhaus-%s" % card_id,
            "source": SOURCE,
            "source_url": url,
            "address": address or ("oberhaus.ee #%s" % card_id),
...
def scrape(
    ...
    html = cached_fetch(
        "oberhaus_ee", query, page_limit, lambda: fetch_search_html(query, page_limit),
