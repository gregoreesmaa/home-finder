"""okidoki.ee listings adapter (general classifieds, incl. real estate).

GitHub issue: #9 ([portal] okidoki.ee adapter).
...
rooms, area_m2}. Selectors live in SELECTORS for one-spot fixes. okidoki is a
general classifieds board (not RE-specific): non-RE cards omit rooms/area_m2
(they stay None) and the title doubles as the address fallback.

Live probe (2026-09-08, polite UA, single GET):
- GET https://www.okidoki.ee/buy/ -> HTTP 200, ~85KB, title
  "Kuulutused – okidoki". Clean category path used: robots.txt disallows
  filtered search URLs with query params (rooms/city/price/sort/order), so
  fetch sticks to the plain category path, page_limit=1.
- Card-level hooks below are best-effort from public markup knowledge plus
  the verified category page; re-verify with one probe if parse yields 0
  rows. No retry, no scraped dumps — fixtures only (hand-built).
...
SOURCE = "okidoki.ee"
BASE_URL = "https://www.okidoki.ee"
# Clean category path (verified HTTP 200 on probe); no filter query params —
# robots.txt disallows them.
SEARCH_URL = BASE_URL + "/buy/"
ROBOTS_URL = BASE_URL + "/robots.txt"

SELECTORS = {
    # Classified ad-card hooks; re-verify against live markup if parse
    # yields 0 rows. Fixture models this card shape.
    "card": r'<div[^>]*class="[^"]*ad-card[^"]*"[^>]*data-id="(?P<id>\d+)"[^>]*>(?P<body>.*?)</div>\s*(?=<div[^>]*class="[^"]*ad-card|$)',
...
def fetch_search_html(query: str = "", page_limit: int = 1, timeout: float = 20.0) -> str:
    """Single page of okidoki.ee category HTML. Keep page_limit small; cron, don't hammer."""
    params = {"page": 1}
    if query:
        params["q"] = query
...
def _parse_card(card_id: str, body: str) -> dict:
    ...
    return normalize_listing(
        {
            "id": "okidoki-%s" % card_id,
            "source": SOURCE,
            "source_url": url,
            "address": address or ("okidoki.ee #%s" % card_id),
...
def scrape(
    ...
    html = cached_fetch(
        "okidoki_ee", query, page_limit, lambda: fetch_search_html(query, page_limit),
