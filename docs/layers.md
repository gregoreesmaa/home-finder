...
| `canopy` (P4 võra, #620) | Maa- ja Ruumiamet CHM WMS `CHM2022_suvi` (CC BY 4.0, 2022 summer flight, taste-only, attributed) | ONE harvest GetMap (2026-09-17): **1000x570 class grid** in the snapshot sidecar (`canopy/canopy-tint.json`: 244k/570k canopy cells, 10-20 m dominant 47 %, >30 m 4 cells, unmatched 0, ~0.8 MB; EPSG:3301 render reverse-sampled to lon/lat by exact forward TM, <1 m at Tallinn) | taste-only class tint (publisher greens + amber/red, <1 m transparent = missing, never short canopy — never a score); NO scorer legs (tint first, capped taste legs second); missing sidecar renders honestly-empty | `lib/layers_p4_canopy.ts` + `scripts/build/batch_canopy.py` |
