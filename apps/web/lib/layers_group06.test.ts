...
describe("batch G06 registry (#138)", () => {
  ...
  it("merges into LAYERS via the G06-HOOK (page + routes serve all fifty-three)", () => {
    ...
    expect(ids.length).toBe(124); // OSMDAILY-HOOK (#482): +6 daily-life layers (77 + 6) + GTFS-HOOK (#483): +1 stop overlay; RSAFE-HOOK (#481): +1 road-safety layer (84 + 1); P4-031-HOOK (#484): +senscom (85 + 1); STATKOV-HOOK (#485): +3 choropleth layers (86 + 3); PLANKTPR-HOOK (#492): +1 designated-use polygon layer (104 + 1); ASUMEDIA-HOOK (#495): +1 per-asum median layer (106 + 1); PAASTE-HOOK (#493): +1 komando overlay (107 + 1); SPORT-HOOK (#607): +3 sport slices (108 + 3); EHIS-HOOK (#608): +3 school slices (111 + 3); MEDRE-HOOK (#609): +2 care slices (114 + 2); OHUSEIRE-HOOK (#610): +1 station dots (116 + 1); KLIIMA-HOOK (#611): +2 climate slices (117 + 2); POI-HOOK (#612): +3 long-tail slices (119 + 3); FIXIT-HOOK (#623): +1 pins layer (122 + 1); QUARRY-HOOK (#614): +1 permit/watch-polygon layer (123 + 1)
