...
// G05A-HOOK(#161): batch G05A (Group 5 plans-A ehitus + korterstock)
// tables live in ./layers_group05a (new file). That module imports
// layers only as types, so no runtime cycle.
import type { Group05ALayerId } from "./layers_group05a";
import {
  GROUP05A_DECAY,
  GROUP05A_LAYERS,
  GROUP05A_TAGS,
...
// G05F-HOOK(#166): batch G05F (Group 5 plans-F upcycle; p389
// documented no-map) tables live in ./layers_group05f (new file).
// That module imports layers only as types, so no runtime cycle.
import type { Group05FLayerId } from "./layers_group05f";
import {
  GROUP05F_DECAY,
  GROUP05F_LAYERS,
  GROUP05F_TAGS,
